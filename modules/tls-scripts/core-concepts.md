...
## Background on TLS
Using self-signed certs with your app involves providing the TLS certificates to the process that is serving requests for your app, so that it can use them to provide a secure connection to clients. 

If you're looking to understand how TLS works at a deeper level, here's: 
* [A written primer on the various entities and components involved in TLS](https://github.com/hashicorp/terraform-aws-vault/tree/master/modules/private-tls-cert#background)
* [a handy illustrated guide to the entire TLS process.](https://tls13.ulfheim.net/) 

## How do I use these certs with my app? 
Using these certs with your app involves providing them to the app process that will serve HTTPS (a web server) so that the server can use them to setup secure connections with clients.
...
We'll first explore the local option.
...
#### Nginx
The following example nginx config demonstrates how to provide the web server with the required certificate and certificate key: 
...
```
/**
 * Start the server to listen on HTTPS, if HTTPS is enabled via the server.httpsPort config.
 * @returns {Server|undefined} - The server instance if HTTPS is enabled or undefined otherwise.
 */
const startHttpsServer = exports.startHttpsServer = () => {
  if (!hasConfig("server.httpsPort")) {
    console.log("server.httpsPort is not defined, so will not listen on HTTPS.");
    return undefined;
  }
  ...
  // Load the TLS certs.
  //
  // In dev, the TLS certs for this app should be generated during boot by bin/run-app.sh. In prod, the TLS certs should
  // be loaded from a secrets manager (e.g., AWS Secrets Manager, HashiCorp Vault, Kubernetes Secrets). In either case,
  ...
  httpsServer.listen(httpsPort, host, () => console.log(`aws-sample-app listening on https://${host}:${httpsPort}!`));
...
```
TODO - add example of parsing the JSON returned from the above function
```
