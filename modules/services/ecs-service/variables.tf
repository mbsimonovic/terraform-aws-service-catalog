...
variable "create_cloudwatch_log_group" {
  description = "When true, create and manage the CloudWatch Log Group in the Terraform module instead of relying on ECS. This is useful for configuring options that are not available in the ECS native feature of managing the Log Group (e.g., encryption support)."
  type        = bool
  default     = false
}

variable "cloudwatch_log_group_name" {
  description = "The name for the Cloudwatch logs that will be generated by the ecs service. Only used (and required) if var.create_cloudwatch_log_group is true."
  type        = string
  default     = null
}

variable "cloudwatch_log_group_retention" {
  description = "Number of days to retain log events. Possible values are: 1, 3, 5, 7, 14, 30, 60, 90, 120, 150, 180, 365, 400, 545, 731, 1827, 3653, and 0. Select 0 to never expire. Only used if var.create_cloudwatch_log_group is true."
  type        = number
  default     = null
}

variable "cloudwatch_log_group_kms_key_id" {
  description = "The ARN of a KMS CMK to use for encrypting log events in the CloudWatch Logs. Set to null to disable encryption. Only used if var.create_cloudwatch_log_group is true."
  type        = string
  default     = null
}
