...
output "public_hosted_zone_map" {
  ...
  value = { for domain, zone in var.public_zones :
    domain => zone.created_outside_terraform ? (zone.hosted_zone_domain_name != "" ? data.aws_route53_zone.selected[zone.hosted_zone_domain_name].zone_id : data.aws_route53_zone.selected[domain].zone_id) : aws_route53_zone.public_zones[domain].id
  }
}
