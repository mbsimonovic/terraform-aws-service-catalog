...
bastion host as a "jump host" to connect to other instances, port forwarding, and more, see the https://github.com/gruntwork-io/terraform-aws-server/tree/master/examples/bastion-host[bastion-host
documentation] documentation in the https://github.com/gruntwork-io/terraform-aws-server[module-server] repo.


=== The bastion host AMI

The bastion host AMI is defined using the https://www.packer.io/[Packer] templates `bastion-host-ubuntu.json` (Packer < v1.7.0) and `bastion-host-ubuntu.pkr.hcl` (Packer >= v1.7.0). The template configures the AMI to:
...
1. Optionally run the https://github.com/gruntwork-io/terraform-aws-monitoring/tree/master/modules/logs/syslog[syslog module] to automatically rotate and rate limit syslog so that the bastion host doesn't run out of disk space from large volumes of logs.
...
* https://github.com/fail2ban/fail2ban[`fail2ban`] bans IP addresses that cause too many failed login attempts. The bastion host is configured with `fail2ban` as a security measure. See the  https://github.com/gruntwork-io/terraform-aws-security/tree/master/modules/fail2ban[`fail2ban` module] for more information.
