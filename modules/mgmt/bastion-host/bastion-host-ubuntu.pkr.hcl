...
source "amazon-ebs" "bastion" {
  ami_description             = "An Ubuntu AMI that is meant to be run as a bastion host."
...
build {
  ...
  provisioner "shell" {
    ...
    inline = [
      ...
      "gruntwork-install --module-name mgmt/bastion-host --repo https://github.com/gruntwork-io/terraform-aws-service-catalog --tag ${var.service_catalog_ref}"
    ]
    pause_before = "30s"
  }
}
