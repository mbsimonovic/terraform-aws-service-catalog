...
variable "ami_name" {
  ...
  type        = string
  default     = "ecs-deploy-runner-worker"
}
...
variable "module_ecs_version" {
  type        = string
  description = "The version of gruntwork-io/terraform-aws-ecs to use."
  default     = ""
}
...
source "amazon-ebs" "ecs-deploy-runner" {
...
build {
  sources = ["source.amazon-ebs.ecs-deploy-runner"]

  provisioner "shell" {
    environment_vars = [
      "GITHUB_OAUTH_TOKEN=${var.github_auth_token}",
      "module_ecs_version=${var.module_ecs_version}",
    ...
    inline = [
      "sudo yum update -y && sudo yum install -y unzip jq",
