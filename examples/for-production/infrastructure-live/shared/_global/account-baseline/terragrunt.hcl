...
inputs = {
  ...
  kms_customer_master_keys = {
    ...
    shared-secrets = {
      ...
      cmk_administrator_iam_arns = ["arn:aws:iam::${local.accounts.shared}:root"]
      cmk_user_iam_arns = [{
        name       = ["arn:aws:iam::${local.accounts.shared}:root"]
        conditions = []
      }]
    ...
    ami-encryption = {
      ...
      cmk_user_iam_arns = [{
        name = [
          ...
          "arn:aws:iam::${local.accounts.shared}:role/aws-service-role/autoscaling.amazonaws.com/AWSServiceRoleForAutoScaling",
        ]
        conditions = []
      }]
      cmk_external_user_iam_arns = [
  ...
  ebs_opt_in_regions        = [local.region_vars.locals.aws_region]
}