...
# https://github.com/gruntwork-io/terraform-aws-service-catalog/blob/v0.65.0/modules/mgmt/ecs-deploy-runner/ecs-deploy-runner-worker-al2.pkr.hcl
#
...
logs_account_id="$(jq -r '."logs".id' "$git_repo_root/accounts.json")"
prod_account_id="$(jq -r '."prod".id' "$git_repo_root/accounts.json")"
security_account_id="$(jq -r '."security".id' "$git_repo_root/accounts.json")"
