...
provider "aws" {
  ...
  # Skip credential validation and account ID retrieval for disabled or restricted regions
  skip_credentials_validation = contains(coalesce(var.kms_grant_opt_in_regions, []), "ap-southeast-3") ? false : true
  skip_requesting_account_id  = contains(coalesce(var.kms_grant_opt_in_regions, []), "ap-southeast-3") ? false : true
}
